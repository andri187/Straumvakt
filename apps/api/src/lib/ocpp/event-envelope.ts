/**
 * Gateway → API Worker event envelope.
 *
 * The OCPP gateway translates OCPP 1.6J / 2.0.1 messages into named
 * domain events (Sprint 1.2) and POSTs them to /api/internal/ocpp-events
 * via Cloudflare Service Binding (ADR 0004). Each call carries exactly
 * one event, already in domain vocabulary — this route never sees OCPP
 * vocabulary (Architecture V3 §2 principle 4).
 *
 * Ported from src/lib/ocpp/event-envelope.ts to apps/api per Sprint S1
 * (gbtNotes/2026-05-01-ocpp-ingest-current-state-and-gaps.md). The
 * envelope shape is unchanged byte-for-byte to preserve idempotency
 * parity with any in-flight gateway events at the moment of cutover.
 *
 ...
 * without the API Worker having to classify them from the event type.
 */
