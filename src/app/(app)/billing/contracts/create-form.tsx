...
export function CreateContractForm({
  ...
  const [scopeType, setScopeType] = useState<typeof SCOPES[number]>("org");
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setDisplayName(""); setScopeId(""); setValidUntil("");
      router.refresh();
    } catch (err) {
  ...
  const valid = orgId && displayName.length > 0 && validFrom.length === 10;
