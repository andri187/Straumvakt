...
const I = {
  dash: <svg viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="3" width="7" height="7" rx="1.5" /><rect x="14" y="14" width="7" height="7" rx="1.5" /><rect x="3" y="14" width="7" height="7" rx="1.5" /></svg>,
  ...
  key: <svg viewBox="0 0 24 24"><circle cx="7.5" cy="15.5" r="4.5" /><path d="m10.5 12.5 8-8M16 7l3 3" /></svg>,
  ...
  if (!ready) {
    return (
      <div className="host-root">
        <div className="login-wrap"><div className="sub">Hleð…</div></div>
      </div>
    );
  }
      ...
      <div className="shell">
        <aside className="sidebar">
          <div className="brand">
            ...
            <div className="word"><strong>Straumvakt</strong><span>Ökumaður · Driver</span></div>
          </div>
          <nav className="nav">
            {SECTIONS.map((sec) => (
              <div key={sec.label}>
                ...
                {sec.items.map((it) => {
                  ...
                  return it.href ? (
                    <Link key={it.label} href={it.href} className={"nav-item" + (active ? " active" : "")}>{inner}</Link>
                  ) : (
                    <div key={it.label} className="nav-item" style={{ opacity: 0.65 }}>{inner}</div>
                  );
                })}
              </div>
            ))}
          ...
          <div className="nav-foot">
            ...
            <div>
              <div style={{ color: "var(--soft)", fontWeight: 600 }}>{name}</div>
              <div>Ökumaður</div>
            </div>
          </div>
        </aside>
        ...
        <div className="main">
          <div className="topbar">
            ...
            <button className="tlink" onClick={logout} style={{ background: "none", border: 0 }}>Útskrá</button>
          </div>
