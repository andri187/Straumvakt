...
export default function DriverCost() {
  ...
  const totalEnergy = rows ? Math.round(rows.reduce((s, h) => s + (h.energyKwh || 0), 0) * 10) / 10 : 0;
  const count = rows?.length ?? 0;

  const byHome = new Map<string, { count: number; energy: number; cost: number }>();
  for (const h of rows ?? []) {
  ...
  return (
    <>
      <div className="head">
        <div>
          <h1>Kostnaður</h1>
          <p>Yfirlit yfir kostnað þinn (síðustu 100 hleðslur), skipt eftir greiðsluheimili.</p>
        </div>
      </div>
      ...
      <div className="grid g3" style={{ marginBottom: 16 }}>
        <div className="card kpi"><div className="lbl">Kostnaður alls</div><div className="val grad">{rows ? kr(totalCost) : "—"}</div></div>
        <div className="card kpi"><div className="lbl">Orka alls</div><div className="val">{rows ? `${totalEnergy.toLocaleString("is-IS")} kWh` : "—"}</div></div>
        <div className="card kpi"><div className="lbl">Hleðslur</div><div className="val">{rows ? count : "—"}</div></div>
      </div>
      ...
      <div className="card">
        ...
        <table>
          <thead>
            <tr><th>Greiðsluheimili</th><th>Hleðslur</th><th>Orka</th><th>Kostnaður</th></tr>
          </thead>
          <tbody>
            {rows === null && <tr><td colSpan={4} className="empty">Hleð…</td></tr>}
            {rows?.length === 0 && <tr><td colSpan={4} className="empty">Engar hleðslur enn.</td></tr>}
            {homes.map(([name, v]) => (
              <tr key={name}>
