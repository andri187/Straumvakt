...
export default function HostDashboard() {
    ...
    (async () => {
      try {
        const [ch, si, ins, se] = await Promise.all([
          apiFetchJson<{ chargers: Charger[] }>(
            `/api/admin/orgs/${org.orgId}/chargers`,
          ),
          apiFetchJson<{ sites: Named[] }>(
            `/api/admin/orgs/${org.orgId}/sites`,
          ),
          apiFetchJson<{ installations: Named[] }>(
            `/api/admin/orgs/${org.orgId}/installations`,
          ),
          apiFetchJson<{ summary: SessionSummary }>(
            `/api/admin/orgs/${org.orgId}/sessions`,
          ),
        ]);
        if (cancelled) return;
  ...
  const countByInst = (name: string) =>
    chargers?.filter((c) => (c.installationDisplayName ?? "") === name)
      .length ?? 0;
    ...
    <>
      <div className="head">
        <div>
          <h1>Mælaborð</h1>
          <p>
            Yfirlit yfir hleðslunet {org?.displayName ?? ""} — þennan mánuð.
          </p>
        </div>
      </div>
      ...
      <div className="grid g4" style={{ marginBottom: 16 }}>
        <Kpi
          lbl="Orka í mánuðinum"
          val={
            sessions
              ? `${sessions.monthEnergyKwh.toLocaleString("is-IS")} kWh`
              : undefined
          }
          grad
        />
        <Kpi
          lbl="Hleðslur í mánuðinum"
          val={sessions ? String(sessions.monthCount) : undefined}
        />
        <Kpi
          lbl="Stöðvar"
          val={chargers === null ? undefined : `${online} / ${total}`}
          sub="nettengdar"
        />
        <Kpi
          lbl="Hleðslur alls"
          val={
            sessions ? sessions.totalCount.toLocaleString("is-IS") : undefined
          }
        />
      </div>
            ...
            <div className="conns">
              {chargers.slice(0, 24).map((c) => (
                <div
                  ...
                  className={"conn " + (c.online ? "ok" : "bad")}
                >
                  <div>
                    <div className="id">
                      {(c.identityString ?? c.chargingStationId).slice(-4)}
                    </div>
