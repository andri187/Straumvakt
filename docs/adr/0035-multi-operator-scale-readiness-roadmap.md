...
| F22 | **Per-day partitions cannot express per-class retention — so F5 is not actually solved.** Partitions are keyed by day; retention is per class; and almost every day's partition holds a mix including `operational`, which has no expiry. The fail-closed gate built in P4.15 is therefore correct and will almost never fire, so `raw_protocol` rows are never reclaimed by dropping partitions. **This is the same defect as the R2 key scheme, one layer down: retention class is absent from the partitioning key.** Reclaiming needs either a class-scoped `DELETE` inside partitions or sub-partitioning by retention class — an ADR-level decision. **F5 (unbounded Postgres growth) remains open.** | `lib/db/partition-cron.ts` |
| F23 | **Two diverging Prisma schemas.** `prisma/schema.prisma` (187 KB) and `apps/api/prisma/schema.prisma` (163 KB) are separate copies, 24 KB apart. Nothing breaks today because the hot paths use raw SQL, but a model added to one is invisible to the other and the gap is widening. | `prisma/`, `apps/api/prisma/` |
| F24 | **A fresh DO during an outage admits everyone at an armed installation.** F21's fix remembers the last-known `enforceAuthorize` in DO storage, but a charger connecting to a *new* DO — first connection, or storage lost — has no memo. Until its first successful lookup lands it behaves as shadow mode and admits every idTag, even at an installation that has enforcement armed. Small and self-healing, but not zero. **The fix is not in the DO**: push `enforceAuthorize` into it at WebSocket-upgrade time, where the entry Worker already resolves the identity and passes `x-straumvakt-identity-id` / `-org-id` / `-identity-string`. Adding the flag there closes the window entirely. Entry-Worker + `apps/api` change; wants its own milestone. | `gateway/src/index.ts`, `identity-do.ts` |
| F21 | ✅ **RESOLVED 2026-08-02** — the gate now fails closed. Operator decision: *"if things are offline, the user can't charge."* No per-installation opt-out. A cached verdict is honoured (cached `Accepted` proceeds, cached `Blocked` refuses, so the Authorize→StartTransaction pair survives an API drop between frames); a genuine miss with upstream unreachable refuses with `Blocked` and a distinct `availabilityRefusal` log field. Last-known `enforceAuthorize` is memoised in DO storage — deliberately *outside* the `authz:` prefix, so `POST /invalidate-authorize {all:true}` (exactly what an operator calls when arming the gate) does not immediately erase the record that it is armed. Residual gap tracked as **F24**. Original finding: | `gateway/src/identity-do.ts:585` |
| ~~F21~~ | **The Authorize gate fails OPEN on upstream error, even when enforced.** `responseFor` returns the stub when the verdict is null, and the stub for `Authorize` is `Accepted` — so if the API Worker or Postgres is unreachable, every idTag is admitted regardless of `enforceAuthorize`. Pre-existing (surfaced while implementing P4.18, not introduced by it). It means flipping enforcement in **P4.11** buys a gate that opens whenever the API is down. Since P4.18 now caches verdicts, serving a stale non-Accepted verdict during an outage is available and strictly safer than fabricating an Accept — but that is an access-precedence change and needs its own Rule 5 pass. | `gateway/src/identity-do.ts:585` |
